...
class SplitAdHull(AdvancedHull):
    ...
    def get_xz_from_y(self, y):
        """
        根据y值获取xz值，注意只有左边的点，单位是格
        :param y: 单位是格
        :return:
        """
        xz = []
        if y < self.y_range[0] or y > self.y_range[1]:
            print(f'警告：y值超出范围，y值应该在{self.y_range[0]}~{self.y_range[1]}之间，当前值为{y}。')
            y = 1
        for key, value in self.SlicesPoints.items():
    ...
    @staticmethod
    def get_plot_triangles(front_dots: List[tuple], back_dots: List[tuple], mode="normal"):
        """
        获取两个截面之间用于绘图的三角点集合，采用NavalArt的坐标系，z是前后，y是上下，x是左右。
        点的排序法是y上到下
        :param front_dots: 前一半的点，左侧（x>0），点为元组
        :param back_dots: 后一半的点，左侧（x>0），点为元组
        :param mode: 模式，front是前一半，back是后一半
        :return:
        """
        # 把点按照y值从上到下排序
        front_dots.sort(key=lambda x: x[1])
        back_dots.sort(key=lambda x: x[1])
        # 判断哪个截面的点少
        less_dots = None
        more_dots = None
        # 初始化结果（加入船底）
        ...
        if len(front_dots) < len(back_dots):
            less_dots = front_dots
            more_dots = back_dots
        elif len(front_dots) > len(back_dots):
            less_dots = back_dots
            more_dots = front_dots
        elif len(front_dots) == len(back_dots):
            # 直接添加三角形然后返回
        ...
        for ld in less_dots:
            ...
            nearest = more_dots[0]
            for md in more_dots:
                if SplitAdHull.distance(ld, md) <= SplitAdHull.distance(ld, nearest):
                    # if abs(ld[1] - md[1]) <= abs(ld[1] - nearest[1]):
                    nearest = md
                elif ld[1] == md[1]:
                    nearest = md
                    break
            counter[ld].append(nearest)
        for md in more_dots:
            nearest = (1000, 1000, 1000)
            changed = False
            for ld in less_dots:
                if SplitAdHull.distance(md, ld) <= SplitAdHull.distance(md, nearest):
                    # if abs(md[1] - ld[1]) <= abs(md[1] - nearest[1]):
                    nearest = ld
                    changed = True
            if changed and md not in counter[nearest]:
                counter[nearest].append(md)
        # 检查counter的值是否有跳跃索引的：
        for ld, connections in counter.items():
            indexes = [more_dots.index(connection) for connection in connections]
            indexes.sort()
            for i in range(len(indexes) - 1):
                if indexes[i + 1] - indexes[i] > 1:
                    # 有跳跃索引，说明有点没有连接上，需要添加
                    for j in range(indexes[i] + 1, indexes[i + 1]):
                        counter[ld].append(more_dots[j])

        # 计算三角形
        for i in range(len(less_dots) - 1):
            ld = less_dots[i]
            next_ld = less_dots[i + 1]
            connections = counter[ld]
            next_connections = counter[next_ld]
...
class ReadPTB:
    ...
    def get_parts(self):
        ...
        for part in self.root.find('parts').findall('part'):  # 遍历所有的零件
            ...
            try:
                ...
                position = (
                    round(str2float(part.find("central").attrib['posX']), 1),
                    round(str2float(part.find("central").attrib['posY']), 1),
                    round(str2float(part.find("central").attrib['posZ']), 1)
                )
                scale = (
    ...
    def get_adHull(self):
        ...
        AdH = SplitAdHull(
            ...
            rail2,
            float(adHull_attr['waterLineHeight']),
    ...
    def __str__(self):
        _str = str(
            f"ShipInfo: {self.dict_str(self.result['ShipInfo'])}\n"
            f"Parts: {self.dict_str(self.result['Parts'])}\n"
            f"ShipType: {self.dict_str(self.result['ShipType'])}\n"
            f"ShipCard: {self.dict_str(self.result['ShipCard'])}\n"
            f"CheakCode: {self.dict_str(self.result['CheakCode'])}\n"
            f"CopyWriting: {self.dict_str(self.result['CopyWriting'])}\n"
            f"Collaborators: {self.dict_str(self.result['Collaborators'])}\n"
            f"Lines2: {self.dict_str(self.result['Lines2'])}\n"
            f"painters: {self.dict_str(self.result['painters'])}\n"
            f"appliques: {self.dict_str(self.result['appliques'])}\n"
            f"rebars: {self.dict_str(self.result['rebars'])}\n"
            f"airfixs: {self.dict_str(self.result['airfixs'])}\n"
            f"armorboards: {self.dict_str(self.result['armorboards'])}\n"
            f"seaspyplanes: {self.dict_str(self.result['seaspyplanes'])}\n"
            f"mods: {self.dict_str(self.result['mods'])}\n"
            f"adHull: {self.dict_str(self.result['adHull'])}\n"
        )
        return _str
